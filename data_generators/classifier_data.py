...
class ClassifierData(IbisData):

    def __init__(self, batch_size):
        super(ClassifierData, self).__init__(batch_size)
    ...
    def load_next_batch(self, width, height):
        ...
        classifier_data_ = np.zeros(shape=(dat_size, 2))
        num_read = 0
        for i in range(start, end):
            ...
            if "real" in image_name:
                classifier_data_[num_read][0] = 1
            else:
                classifier_data_[num_read][1] = 1
        ...
        return image_data, classifier_data_, last
